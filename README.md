...
3. [ ] Title의 isSame 메서드가 정말 필요한 메서드인가?
4. [ ] Movie의 isValidTitle 메서드: 영화에게 영화의 제목이 유효한지 물어봐야 하는 이유가 무었인가?
5. [ ] PayCalculator의 각 전략들이 런타임에 계속해서 변화되어야 한다면, 유연하게 대처할 수 있는 구조인가?
6. [ ] PayCalculator Point 관련
   1. [ ] 포인트를 매우 많이 쓰면 앱이 터지고, 에러 로그로 돈은 양수여야 한다고 나옴
   2. [ ] 실제로 totalPrice에 point가 사용되었는가?
   3. [ ] Point라는 객체가 있는데, usePoint 메서드에선 Money를 쓰고 있음. 프로젝트의 일관성이 유지되고 있는가?
