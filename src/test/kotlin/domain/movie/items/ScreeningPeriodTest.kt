...
class ScreeningPeriodTest {
    ...
    private val screeningPeriod = ScreeningPeriod(startDate, endDate)
    ...
    fun `입력 받은 날자가 영화의 상영 기간에 포함된다면 true가 반환된다`() {
        val result = screeningPeriod.isContain(LocalDate.of(2026, 4, 8))
    ...
    fun `입력 받은 날자가 영화의 상영 기간에 포함되지 않는다면 false가 반환된다`() {
        val result = screeningPeriod.isContain(LocalDate.of(2025, 4, 8))
