...
class SeatTest {
    ...
    fun `A1 좌석은 Row A, Column 1, B등급이다`() {
        // given
        val seat = Seat(SeatRow("A"), SeatColumn(1), SeatGrade.B)

        // then
        assert(seat.seatRow == SeatRow("A"))
        assert(seat.seatColumn == SeatColumn(1))
    ...
    fun `좌석행은 비어 있을 수 없다`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Seat(SeatRow(""), SeatColumn(1), SeatGrade.B)
            }
    ...
    fun `좌석행은 A에서 E 사이여야 한다`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Seat(SeatRow("F"), SeatColumn(1), SeatGrade.B)
            }
    ...
    fun `좌석열은 1에서 4 사이어야 한다`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Seat(SeatRow("A"), SeatColumn(5), SeatGrade.B)
            }
