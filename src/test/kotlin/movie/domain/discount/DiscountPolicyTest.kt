...
class DiscountPolicyTest {
    ...
    fun `무비데이에 해당하면 할인된 금액을 반환한다`() {
        ...
        val discountPrice = discountPolicy.calculateMovieDayDiscount(
            totalPrice = screeningMovie.calculatePrice(
    ...
    fun `타임 할인에 해당하면 할인된 금액을 반환한다`() {
        ...
        val discountPrice = discountPolicy.calculateTimeDiscount(
            totalPrice = screeningMovie.calculatePrice(
                targetSeatNumbers = listOf(SeatNumber(Row('A'), Column(1)))
            ),
